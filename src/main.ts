...
type LavaLine = {
  points: { x: number; y: number; sizeMultiplier: number; touchWidth: number; touchHeight: number }[];
  startTime: number;
  isMelting: boolean;
  meltProgress: number;
};
...
function calculateSizeMultiplier(width: number, height: number): number {
  ...
  if (isWebApp) {
    ...
    if (Math.random() < 0.01) { // Log occasionally to avoid spam
      console.log('Web app size calculation:', { baseMultiplier, thicknessMultiplier, finalMultiplier });
    }
    ...
    const smoothedMultiplier = lastSizeMultiplier * (1 - sizeSmoothingFactor) + (baseMultiplier * sizeVariation) * sizeSmoothingFactor;
    lastSizeMultiplier = smoothedMultiplier;
    
    const finalMultiplier = smoothedMultiplier * (sizeMultipliers[currentSizeLevel] || 1.0) * thicknessMultiplier;
    if (Math.random() < 0.01) { // Log occasionally to avoid spam
      console.log('Size calculation:', { smoothedMultiplier, sizeLevel: sizeMultipliers[currentSizeLevel], thicknessMultiplier, finalMultiplier });
    }
    return finalMultiplier; // Apply size control and thickness
  }
}
...
function drawStyle2(ctx: CanvasRenderingContext2D, x: number, y: number, sizeMultiplier: number, touchWidth: number, touchHeight: number): void {
...
function drawFlameEffect(ctx: CanvasRenderingContext2D, x: number, y: number, sizeMultiplier: number, touchWidth: number, touchHeight: number): void {
  // Simple sparkle effect - just draw the sparkle line
  drawStyle2(ctx, x, y, sizeMultiplier, touchWidth, touchHeight);
}

function drawLavaEffect(ctx: CanvasRenderingContext2D, x: number, y: number, sizeMultiplier: number, touchWidth: number, touchHeight: number): void {
...
function _drawAllStyle2Lines(ctx: CanvasRenderingContext2D): void {
  ...
  for (let i = lavaLines.length - 1; i >= 0; i--) {
    ...
    for (let j = 0; j < line.points.length; j++) {
      ...
      if (point) {
        drawLavaEffect(ctx, point.x, point.y, point.sizeMultiplier, point.touchWidth, point.touchHeight);
      }
    }
  }
}
...
function resizeCanvas(canvas: HTMLCanvasElement): void {
  ...
  console.log('Canvas resized to HD:', canvas.width, 'x', canvas.height, 'devicePixelRatio:', devicePixelRatio);
  ...
  canvas.addEventListener('pointerdown', (e: PointerEvent) => {
    ...
    if (ctx) {
      ...
      if (currentStyle === 2) {
        ...
        console.log('Drawing dot at:', x, y, 'size:', touchWidth, 'x', touchHeight, 'pressure:', pressure);
  ...
  canvas.addEventListener('pointermove', (e: PointerEvent) => {
    ...
    for (let i = 0; i <= adjustedNumDots; i++) {
      ...
      if (currentStyle === 2) {
        ...
        ctx.beginPath();
        ctx.ellipse(
          dotX, dotY,                                        // interpolated position
...
function generateRandomStyle1Parameters(): void {
  ...
  console.log(`Generated aggressive pen style: hsl(${hue}, ${saturation}%, ${lightness}%) - Intensity: ${sparkleIntensity.toFixed(2)}`);
}
...
function init(): void {
  ...
  if (thicknessSlider) {
    const thicknessHandler = () => {
      ...
      console.log('🎯 Thickness changed to:', thicknessMultiplier, 'Slider value:', thicknessSlider.value);
